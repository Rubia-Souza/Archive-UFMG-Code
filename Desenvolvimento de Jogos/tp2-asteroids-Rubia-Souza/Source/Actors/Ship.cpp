        ...
        , mFrictionCoefficient(frictionCoefficient)
{
    ...
    Vector2 v1 = Vector2(-mHeight, mHeight / 2);
    Vector2 v2 = Vector2(mHeight, 0);
    Vector2 v3 = Vector2(-mHeight, -mHeight / 2);
...
void Ship::OnProcessInput(const uint8_t* state)
{
    ...
    if (state[SDL_SCANCODE_W]) {
        mRigidBodyComponent->ApplyForce(mForwardSpeed * GetForward());
    }
    ...
    if (state[SDL_SCANCODE_SPACE] && mLaserCooldown <= 0.0f) {
        ...
        laser->SetPosition(Vector2(mPosition.x + GetForward().x, mPosition.y + GetForward().y + mHeight));
        ...
        laser->GetComponent<RigidBodyComponent>()->ApplyForce(3000.0f * GetForward());
...
void Ship::OnUpdate(float deltaTime)
{
    ...
    if(velocity.Length() > 0) {
        ...
        float dragLength = mFrictionCoefficient * velocityLength * velocityLength;

        Vector2 drag = (-1) * velocity;
        drag.Normalize();
        drag *= dragLength;
    ...
    for (Asteroid* asteroid : GetGame()->GetAsteroids()) {
        CircleColliderComponent* asteroidCollider = asteroid->GetComponent<CircleColliderComponent>();
        if (mCircleColliderComponent->Intersect(*asteroidCollider)) {
            GetGame()->Quit();
        }
    }
}
