...
Asteroid::Asteroid(Game* game, const float radius, const int numVertices, const bool isLarge)
        :Actor(game)
        ...
        ,mIsLarge(isLarge)
{
    ...
    Vector2 initialPosition = Random::GetVector(Vector2(GetGame()->GetWindowWidth() / 2 + 100, 0),
                                                Vector2(GetGame()->GetWindowWidth() - 100, GetGame()->GetWindowHeight()));
...
Asteroid::~Asteroid()
{
    ...
    if (mIsLarge) {
        for (int i = 0; i < 3; i++) {
            Asteroid* childAsteroid = new Asteroid(mGame, 45, 10, false);
            childAsteroid->SetPosition(mPosition);
        }
    }

    for (int i = 0; i < 5; i++) {
        Particle* destructionParticle = new Particle(mGame);
        destructionParticle->SetPosition(mPosition);
    }
...
float Asteroid::CalculateAverageVerticesLength(std::vector<Vector2>& vertices)
{
    ...
    for (Vector2 vertice : vertices) {
        averageLength += vertice.Length();
    }
    return averageLength / vertices.size();
}
