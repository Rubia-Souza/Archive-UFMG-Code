...
void DrawComponent::DrawPolygon(SDL_Renderer *renderer, std::vector<Vector2>& vertices)
{
    ...
    for (int i = 1; i < vertices.size(); i++) {
        Vector2 startPoint = pos + vertices[i - 1];
        Vector2 endPoint = pos + vertices[i];
    ...
    Vector2 firstPoint = pos + vertices[0];
    Vector2 lastPoint = pos + vertices[vertices.size() - 1];
...
void DrawComponent::DrawCircle(SDL_Renderer *renderer, const Vector2& center, const float radius, const int numVertices)
{
    ...
    for (int i = 0; i < numVertices; i++) {
        ...
        vertices.emplace_back(x + center.x, y + center.y);
...
void DrawComponent::Draw(SDL_Renderer *renderer)
{
    ...
    for (Vector2 vertice : mVertices) {
        tempVertices.emplace_back(Vector2::Transform(vertice, rotationMatrix));
    }
    ...
    DrawPolygon(renderer, tempVertices);
    ...
    //CircleColliderComponent* collider = mOwner->GetComponent<CircleColliderComponent>();
    //DrawCircle(renderer, Vector2::Zero, collider->GetRadius());
}