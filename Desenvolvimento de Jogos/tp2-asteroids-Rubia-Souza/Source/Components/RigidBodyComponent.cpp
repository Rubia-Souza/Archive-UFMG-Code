...
void RigidBodyComponent::Update(float deltaTime)
{
    ...
    if (Math::NearZero(mVelocity.Length(), 8)) {
...
void RigidBodyComponent::ScreenWrap(Vector2 &position)
{
    ...
    //  igual a zero.
    if (position.x < 0) {
        position.x = GetGame()->GetWindowWidth();
    } else if (position.x > GetGame()->GetWindowWidth()) {
        position.x = 0;
    }
    ...
    //  igual a zero.
    if (position.y < 0) {
        position.y = GetGame()->GetWindowHeight();
    } else if (position.y > GetGame()->GetWindowHeight()) {
        position.y = 0;
    }
}
