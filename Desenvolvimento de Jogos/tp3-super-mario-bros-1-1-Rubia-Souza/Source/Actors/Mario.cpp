        ...
        , mJumpSpeed(jumpSpeed)
{
    ...
    //  no personagem.
    mRigidBodyComponent = new RigidBodyComponent(this, 1.0, 8.0);
    ...
    mDrawComponent = new DrawAnimatedComponent(this, "../Assets/Sprites/Mario/SpriteSheetMario.png", "../Assets/Sprites/Mario/SpriteSheetMario.json");


    // TODO 4.2 (~4 linhas): Utilize a função `AddAnimation` para adicionar as animações "dead", "idle",
    //  "jump" e "run".
    std::vector<int> deadAnimation = {0};
    mDrawComponent->AddAnimation("dead", deadAnimation);
    std::vector<int> idleAnimation = {1};
    mDrawComponent->AddAnimation("idle", idleAnimation);
    std::vector<int> jumpAnimation = {2};
    mDrawComponent->AddAnimation("jump", jumpAnimation);
    std::vector<int> runAnimation = {3, 4, 5};
    mDrawComponent->AddAnimation("run", runAnimation);
...
void Mario::OnProcessInput(const uint8_t* state)
{
    ...
    if (mIsOnGround && state[SDL_SCANCODE_SPACE]) {
        mRigidBodyComponent->SetVelocity(Vector2(mRigidBodyComponent->GetVelocity().x, mJumpSpeed * 1));
        mIsOnGround = false;
    }
}
...
void Mario::OnUpdate(float deltaTime)
{
    ...
    if (GetPosition().y > mGame->GetWindowHeight()) {
        Kill();
    }
