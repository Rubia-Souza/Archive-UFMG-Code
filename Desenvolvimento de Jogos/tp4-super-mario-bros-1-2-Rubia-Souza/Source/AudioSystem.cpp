...
void AudioSystem::Update(float deltaTime)
{
	...
	// TODO [Rubia] Olhar se esta certo
	for (int i = 0; i < mChannels.size(); i++) {
		HandleInfo info = mHandleMap[mChannels[i]];
		if (Mix_Playing(i) == 0) {
...
SoundHandle AudioSystem::PlaySound(const std::string& soundName, bool looping)
{
	...
	if (sound == nullptr) {
		SDL_Log("Failed to get sound %s", soundName.c_str());
	...
	for (int i = 0; i < mChannels.size(); i++) {
		// TODO [Rubia] Verificar se está certo usar a negação
	...
	if (availableChannel == -1) {
		for (auto it = mHandleMap.begin(); it != mHandleMap.end(); it++) {
			if (soundName == it->second.mSoundName) {
				availableChannel = it->second.mChannel;
				SDL_Log("[LOG] Stoping sound: %s", soundName.c_str());
				mHandleMap.erase(it);
				// TODO [Rubia] Deveria parar o som?
				break;
			}
		}
	}
	...
	if (availableChannel == -1) {
		for (auto it = mHandleMap.begin(); it != mHandleMap.end(); it++) {
			if (it->second.mIsLooping) {
				availableChannel = it->second.mChannel;
				SDL_Log("[LOG] Stopping sound in loop: %s", it->second.mSoundName.c_str());
				StopSound(it->first);
				break;
			}
		}
	...
	if (availableChannel == -1) {
		auto targetSound = mHandleMap.begin();
		SDL_Log("[LOG] Stopping oldest sound: %s", targetSound->second.mSoundName.c_str());
		availableChannel = targetSound->second.mChannel;
		StopSound(targetSound->first);
	}
	...
	mHandleMap.insert({mLastHandle, info});
...
void AudioSystem::StopSound(SoundHandle sound)
{
	...
	if (it != mHandleMap.end()) {
		...
		SDL_Log("[ERROR] Couldn't find sound to stop it");
	}
...
void AudioSystem::PauseSound(SoundHandle sound)
{
	...
	if (it != mHandleMap.end()) {
		...
		SDL_Log("[ERROR] Couldn't find sound to pause");
	}
}
...
void AudioSystem::ResumeSound(SoundHandle sound)
{
	...
	if (it != mHandleMap.end()) {
		...
		SDL_Log("[ERROR] Couldn't find sound to resume");
	}
}
