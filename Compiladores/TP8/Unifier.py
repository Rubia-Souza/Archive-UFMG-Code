...
def unify(constraints, sets):
    ...
    type0, type1 = constraints[0]
    rest = constraints[1:]

    if type0 != type1:
        set0 = sets.setdefault(type0, set())
        set1 = sets.setdefault(type1, set())
        new_set = set0 | set1 | {type0, type1}
...
def name_sets(sets):
    ...
    * Ambiguous type: if any canonical type set contains more than one element.
    ...
    # TODO: Implement this method!
    def is_valid(type_set):
        temp = set(type_set)
        for element in temp:
            if not isinstance(element, type):
                type_set.remove(element)
        if len(type_set) == 0 or len(type_set) > 1:
            sys.exit('Type error')

    for type_set in sets.values():
        is_valid(type_set)

    return {value: list(element)[0] for value, element in sets.items()}
