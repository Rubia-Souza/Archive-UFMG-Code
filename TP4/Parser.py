...
class Parser:
    ...
    def Q(self) -> Expression:
        ...
        if self.next_token().kind == TokenType.NUM:
            ...
            return tree
        elif self.next_token().kind == TokenType.TRU:
            tree = Bln(True)
            self.consume()
            return tree
        elif self.next_token().kind == TokenType.VAR:
            tree = Var(self.next_token().text)
            self.consume()
            return tree
        elif self.next_token().kind == TokenType.LET:
            self.consume()
            var: Token = self.next_token()
            self.match(TokenType.VAR)
            self.match(TokenType.ASN)
            value: Expression = self.E()
            self.match(TokenType.INX)
            body: Expression = self.E()
            self.match(TokenType.END)
            return Let(var.text, value, body)
